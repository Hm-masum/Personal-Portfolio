...
const UpdateProjectForm = ({ projectData }: { projectData: IProject }) => {
  const form = useForm({
    defaultValues: {
      ...
      technologies: projectData?.technologies?.map((technology) => ({
        value: technology,
      })) || [{ value: "" }],
    },
  });
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (data) => {
    try {
      ...
      const res = await updateProject(updateData, projectData._id);
      if (res.success) {
